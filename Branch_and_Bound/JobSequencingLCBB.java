//TODO : Job Sequencing using Least Cost Branch and Bound {LCBB}
//Author : Hyder Nabi
...
public class JobSequencingLCBB implements data{
	/*//Penalty
	...
	*/
	...
	//queue
	public static node List[] = new node[100];
		...
		while(f<=r) {
		 ...
		 if(E.TotalPanelty < Answer.TotalPanelty)
		 {
			...
			for(int i=1;i<=n;i++) {
				if(i>E.x) {
					if(isFeasible(i, E)) {
						double cost = Cost(i, E); 
						if( cost < UPPER)//kill node if cost is greater than upper limit
						{
							double upper = Upper(i, E);
							if(upper<UPPER)
							{
								UPPER = upper;//update upper limit
							}
							child = new node();
							child.parent = E;
							child.val = counter++;
	...
	{
		double min = List[f].cost;
		node minNode = List[f];
		int index = f;
		for(int i=f;i<=r;i++)
		{
			if(List[i] !=null && List[i].cost<min)
			{
				min = List[i].cost;
				index = i;
				minNode = List[i];
	...
	public static void main(String args[]) {
		...
		root.TotalPanelty = 22; // 24 for example 1//when no job is selected//sum of penalties of all jobs
